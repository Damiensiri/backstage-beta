(function(){
  ...
  async function load(){status(elements.connection,"Chargement…");try{loadedUsers=await api("/api/admin/users");renderSearch();status(elements.connection,`${loadedUsers.length} compte(s) bêta.`,"success");}catch(error){status(elements.connection,error.message,"error");}}
